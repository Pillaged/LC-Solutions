...
def smol(num_list, S):
    smallestlen = len(num_list)+1
    max = smallestlen
    startpos = 0
    sum_curr = 0
    for endpos in range(0,len(num_list)):
        sum_curr += num_list[endpos]
        while endpos > startpos:
            if sum_curr >= S:
                smallestlen = min(smallestlen, (endpos-startpos)+1)
                sum_curr -= num_list[startpos]
                startpos += 1
            else: break
    if smallestlen < max:
        return smallestlen 
...
def main():
    print(smol([2, 1, 5, 2, 3, 2], S=7))
    print(smol([2, 1, 5, 2, 8], S=7))
    print(smol([3, 4, 1, 1, 6], S=8))
